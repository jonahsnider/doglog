	...
	if (sortedTunables.length === 0) {
		return (
			<div className="rounded-lg border border-[var(--sl-color-gray-5)] bg-[var(--sl-color-gray-6)] p-8 text-center">
				...
				<h3 className="mb-2 text-lg font-medium text-[var(--sl-color-white)]">
					Waiting for tunable values...
				</h3>
				<p className="text-[var(--sl-color-gray-3)]">
					Tunable values created with <code className="text-[var(--sl-color-accent)]">DogLog.tunable()</code> will appear here automatically.
				</p>
			</div>
		);
	}
	...
	return (
		<div className="space-y-4">
			<div className="flex items-center justify-between">
				<h2 className="text-lg font-medium text-[var(--sl-color-white)]">
					Tunable Values ({sortedTunables.length})
				</h2>
			</div>

			{sortedGroups.map(([group, values]) => (
				<div key={group} className="rounded-lg border border-[var(--sl-color-gray-5)] bg-[var(--sl-color-gray-6)] overflow-hidden">
